...
- For graphs and data visualisation: use **D3.js** via CDN.
- For animations: use **anime.js** via CDN.
- Prefer no backend for simple projects — use browser APIs, CDN services, or static hosting whenever possible. Only introduce a Node.js backend when the project genuinely requires it (e.g. auth secrets, database access, server-side logic). When a backend is needed, use Node.js.
...
## Learnings Log
- A `learnings.md` file lives at the project root. Create it if it does not exist.
- After fixing a bug, resolving a tricky issue, or discovering something non-obvious about this project, append a new entry to `learnings.md`.
- Each entry must include: the date, a plain-English summary of the problem, what caused it, and what fixed it.
- Write entries as if briefing the next AI coder who picks up this project — clear enough that they can avoid the same mistake without re-investigating.
- Never delete or rewrite existing entries. Append only.
